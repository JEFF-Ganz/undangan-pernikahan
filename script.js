// -----------------------------------------------------------------
        ...
        if (rsvpForm) {
          rsvpForm.addEventListener('submit', async (e) => {
            ...
            if (!firebaseInitialized || !db) {
              alert('Pesan disimpan secara lokal (mode pratinjau).');
              ...
              card.innerHTML = `<strong>${name}</strong> <small>(${attendance})</small><p>${message}</p>`;
            ...
            const phone = document.getElementById('guests').value;
            const attendance = document.getElementById('attendance').value;
        ...
        if (guestbookContainer) {
          if (!firebaseInitialized || !db) {
            guestbookContainer.innerHTML = `<p style="text-align:center; color:#666; font-style:italic; padding:30px;">Tampilan pesan guestbook sementara.</p>`;
          } else {
